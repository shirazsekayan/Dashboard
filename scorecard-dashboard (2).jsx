...
const ScorecardDashboard = () => {
  ...
  const [defaultMetrics, setDefaultMetrics] = useState([
    { key: 'bankDeposit', label: 'Bank Deposit', icon: '💰', color: '#10b981', prefix: '$', isCustom: false },
    { key: 'production', label: 'Production', icon: '📊', color: '#3b82f6', prefix: '$', isCustom: false },
    { key: 'yelpReview', label: 'Yelp Reviews', icon: '⭐', color: '#ef4444', prefix: '', isCustom: false },
    { key: 'googleReview', label: 'Google Reviews', icon: '⭐', color: '#eab308', prefix: '', isCustom: false },
    { key: 'newPatients', label: 'New Patients', icon: '👥', color: '#a855f7', prefix: '', isCustom: false }
  ]);
      ...
      <div style={{ minHeight: '100vh', background: colors.background, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ width: '100%', maxWidth: '400px', background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '24px', padding: '40px' }}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            <div style={{ display: 'inline-flex', width: '80px', height: '80px', background: 'linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)', borderRadius: '16px', marginBottom: '16px', fontSize: '32px', alignItems: 'center', justifyContent: 'center' }}>📋</div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto 32px' }}>
        <div style={{ background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ width: '56px', height: '56px', background: 'linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px' }}>📋</div>
            <div>
              <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: colors.text, margin: 0 }}>2026 Scorecard Dashboard</h1>
              <p style={{ color: colors.textSecondary, margin: 0 }}>{isAdmin ? '👑 Admin Mode' : '👁️ View Only'}</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            ...
            <button onClick={() => { setView('landing'); setActiveTab('goals'); }} style={{ padding: '10px 20px', background: '#334155', color: '#cbd5e1', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>🚪 Logout</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto 24px' }}>
        <div style={{ background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '8px', display: 'flex', gap: '8px' }}>
          <button onClick={() => setActiveTab('goals')} style={{ flex: 1, padding: '12px 24px', borderRadius: '10px', fontSize: '15px', fontWeight: '600', border: 'none', cursor: 'pointer',
            background: activeTab === 'goals' ? 'linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)' : 'transparent', color: colors.text }}>🎯 Quarterly Goals</button>
          <button onClick={() => setActiveTab('weekly')} style={{ flex: 1, padding: '12px 24px', borderRadius: '10px', fontSize: '15px', fontWeight: '600', border: 'none', cursor: 'pointer',
            background: activeTab === 'weekly' ? 'linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%)' : 'transparent', color: colors.text }}>📅 Weekly Check-ins</button>
        </div>
      </div>
          ...
          {isAdmin && (
            <div style={{ maxWidth: '1400px', margin: '0 auto 32px' }}>
              <div style={{ background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '16px', display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button onClick={() => setShowCustomMetricModal(true)} style={{ padding: '10px 24px', background: '#8b5cf6', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>➕ Add Custom Widget</button>
                  <button onClick={() => setShowManageWidgetsModal(true)} style={{ padding: '10px 24px', background: '#6366f1', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>⚙️ Manage Widgets</button>
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                  {!editMode ? (
                    <button onClick={() => { setEditValues({ ...currentGoals }); setEditMinValues({ ...currentMin }); setEditMaxValues({ ...currentMax }); setEditMode(true); }}
                      style={{ padding: '10px 24px', background: '#3b82f6', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>✏️ Edit Goals, Min & Max</button>
                  ) : (
                    <div style={{ display: 'flex', gap: '12px' }}>
                      ...
                      <button onClick={() => { 
                        ...
                        setData(newData); 
                        setEditMode(false); 
                      }} style={{ padding: '10px 24px', background: '#10b981', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '500' }}>💾 Save All</button>
                    </div>
                  )}
                </div>
              </div>
                ...
                return (
                  <div key={metric.key} style={{ background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
                      ...
                      {editMode && isAdmin && <span style={{ fontSize: '20px' }}>✏️</span>}
                    </div>
      ...
      {activeTab === 'weekly' && (
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          <div style={{ background: colors.cardBg, backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '24px', marginBottom: '24px' }}>
            <h2 style={{ color: colors.text, fontSize: '18px', fontWeight: '600', margin: '0 0 20px 0' }}>📊 Progress Towards {selectedQuarter} Goals</h2>
          ...
          {isAdmin && (
            <div style={{ marginBottom: '24px' }}>
              <button onClick={addWeeklyCheckIn} style={{ padding: '12px 24px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: colors.text, borderRadius: '12px', border: 'none', cursor: 'pointer', fontSize: '15px', fontWeight: '600' }}>➕ Add Weekly Check-in</button>
            </div>
          )}
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {weeklyEditMode && selectedWeek === weekKey ? (
                            <>
                              ...
                              <button onClick={saveWeeklyEdit} style={{ padding: '8px 16px', background: '#10b981', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>💾 Save</button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => startWeeklyEdit(weekKey)} style={{ padding: '8px 16px', background: '#3b82f6', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>✏️ Edit</button>
                              {deleteConfirm === weekKey ? (
                                <>
                                  <button onClick={() => handleDeleteWeek(weekKey)} style={{ padding: '8px 16px', background: '#dc2626', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '700' }}>✓ Confirm</button>
                                ...
                                <button onClick={() => setDeleteConfirm(weekKey)} style={{ padding: '8px 16px', background: '#ef4444', color: colors.text, borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>🗑️ Delete</button>
                              )}
                            </>
                          )}
                        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 1000 }}>
          <div style={{ background: 'rgba(30, 41, 59, 0.95)', backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '32px', maxWidth: '500px', width: '100%' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              <div style={{ width: '48px', height: '48px', background: 'rgba(139, 92, 246, 0.2)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' }}>➕</div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px' }}>
                {availableColors.map(colorOption => (
                  ...
                  >
                    {newMetric.color === colorOption.value && '✓'}
                  </button>
                ))}
              </div>
            </div>
      ...
      {showManageWidgetsModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', zIndex: 1000 }}>
          <div style={{ background: 'rgba(30, 41, 59, 0.95)', backdropFilter: 'blur(10px)', border: `1px solid ${colors.cardBorder}`, borderRadius: '16px', padding: '32px', maxWidth: '600px', width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
              <div style={{ width: '48px', height: '48px', background: 'rgba(99, 102, 241, 0.2)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px' }}>⚙️</div>
            ...
            <div style={{ borderTop: '1px solid rgba(255, 255, 255, 0.1)', paddingTop: '16px', marginTop: '16px' }}>
              <p style={{ color: '#94a3b8', fontSize: '13px', marginBottom: '16px' }}>
                💡 Use arrow buttons to reorder any widget. The 🗑️ button deletes widgets (both default and custom). <strong style={{ color: '#cbd5e1' }}>Changes apply to all quarters!</strong>
              </p>
              <button 
